...
open class ItemSkillsJeiPlugin : IModPlugin {
  ...
  override fun getPluginUid(): ResourceLocation {
    return ResourceLocation(ItemSkills.MOD_ID, "jei_plugin")
  }
  ...
  private fun processUnconsumables() {
    ...
    if (toShow.isNotEmpty()) {
      ItemSkills.LOGGER.debug(
        "Showing ${toShow.size} item(s) based on consumables: ${toShow.map { ItemRestriction.getName(it) }}",
      )
    ...
    if (toHide.isNotEmpty()) {
      ItemSkills.LOGGER.debug(
        "Hiding ${toHide.size} item(s) based on consumables: ${toHide.map { ItemRestriction.getName(it) }}",
      )
  ...
  private fun processUnproducibles() {
    ...
    if (toShow.isNotEmpty()) {
      ...
      ItemSkills.LOGGER.debug(
        "Showing ${toShow.size} item(s) based on producibles: ${toShow.map { ItemRestriction.getName(it) }}",
      )
    ...
    if (toHide.isNotEmpty()) {
      ...
      ItemSkills.LOGGER.debug(
        "Hiding ${toHide.size} item(s) based on producibles: ${toHide.map { ItemRestriction.getName(it) }}",
      )
